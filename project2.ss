...
(define getNonterminals
  (lambda (grammar)
    (getNonterminalsHelper grammar '())
  ))

;; The way that I wanted to implement this is by

(define getNonterminalsHelper
  (lambda (grammar l)
    (if(null? grammar) '()
       (if (not (and (null? (caar grammar)) (null? (cdr grammar))))
       (cons (append l (caar grammar)) (getNonterminalsHelper (cdr grammar) l))))))
...
(define getProductions
  (lambda (grammar)
    (if(null? grammar)
       '())
    (getProductionsHelper '() grammar)))

(define getProductionsHelper
  (lambda (l grammar)
    (if (null? grammar)
        '()
        (if (and (null? (caar grammar)) (null? (cdr grammar)) (null? (cadr grammar))) '()
            (append (singleLineHelper (caar grammar) (cdar grammar)) (getProductionsHelper l (cdr grammar)))))))

(define singleLineHelper
  (lambda (symbol rest)
    (if (null? rest) '()
        (append (cons (cons symbol (list-ref rest 0)) '()) (singleLineHelper symbol (cdr rest))))))
...
(define updateAssocList
  (lambda (assocList symbol set)
    (if (null? assocList) '()
        (if (not (contains? (car assocList) symbol))
            (cons (car assocList) (updateAssocList (cdr assocList) symbol set))
            (cons (cons symbol (cons (union (cadar assocList) set) '())) (cdr assocList))))))
  ...
  (lambda (NTs startSymbol setting)
    (if (null? NTs) '()
        (if (eq? (car NTs) startSymbol)
            (if(eq? setting 'follow)
               (cons (cons (car NTs) (cons '(eof) '())) (getInitSets (cdr NTs) startSymbol setting))
               (cons (cons (car NTs) '(())) (getInitSets (cdr NTs) startSymbol setting)))
        (cons (cons (car NTs) '(())) (getInitSets (cdr NTs) startSymbol setting))))))
    ...
    (letrec ((first 
              (lambda (symbolSeq firstSets)
                (if(null? symbolSeq) '()
                   ...
                   (if (not (contains? NTs (car symbolSeq)))
                       (list (car symbolSeq))
                       ...
                       (if(contains? (cadr (assoc (car symbolSeq) firstSets)) (epsilon))
                          ...
                          (if (null? (cdr symbolSeq))
                              ...
                              (union (removeMatch (cadr (assoc (car symbolSeq) firstSets)) (epsilon)) (first (cdr symbolSeq) firstSets))) 
...
(define recurseFirstSets
 (lambda (rules firstSets firstFunc)
   (if (null? rules) firstSets
        
         ;; If the rules are not null, then you can
         ;; recurse through the rules and send each
         ;; line of the production rule into the genFirstFunc
         ;; as the symbol sequence for that production rule
         ;; YOUR CODE GOES HERE. You may delete this pair of parentheses if necessary.
         ;; Maybe do removeMatch
        (let ((rhsRule (cdar rules)) 
              (lhsRule (caar rules)))
              ...
              (lambda (symbolSeq followSets trailer firstSets)
                (if (null? symbolSeq)
                    ...
                    (if (not (contains? NTs (car symbolSeq)))
                        ...
                        (let ((newFollow (updateAssocList followSets (car symbolSeq) trailer)))
                          (if (contains? (cadr (assoc (car symbolSeq) firstSets)) (epsilon))
...
(define recurseFollowSets
  (lambda (rules followSets firstSets followFunc)
    (if (null? rules) followSets
        (let* ((rhsRule (reverse (cdar rules)))
               (lhsRule (caar rules))
    ...
    (if (null? rules)
       ...
       (map
        (lambda (x)
          (let* ((rhsRule (cdr x))
                 (lhsRule (car x))
                 (firstRHS (firstFunc rhsRule firstSets))
                 (followSymbol (cadr (assoc lhsRule followSets))))

                 (if (contains? firstRHS (epsilon))
                     (cons (list lhsRule '::= rhsRule) (cons (union (removeMatch firstRHS (epsilon)) followSymbol)'()))
                     (cons (list lhsRule '::= rhsRule) (cons firstRHS '())))))
